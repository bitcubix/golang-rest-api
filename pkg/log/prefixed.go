...
type textFormatter struct {
	...
	// Whether the log's out is to a terminal.
	isTerminal bool
...
func (f *textFormatter) Format(entry *logrus.Entry) ([]byte, error) {
	...
	if isFormatted {
		...
		level := entry.Level.String()[0]
		f.appendKeyValue(b, "level", level, true)
		if entry.Message != "" {
...
func (f *textFormatter) printColored(b *bytes.Buffer, entry *logrus.Entry, keys []string, timestampFormat string, colorScheme *compiledColorScheme) {
	...
	if prefixValue, ok := entry.Data["prefix"]; ok {
		prefixValue = fmt.Sprintf("%-12s", prefixValue)
