...
func New(driver, dataSource string, logger log.Logger) (*Connection, error) {
	logger.WithFields(log.Fields{"driver": driver, "source": dataSource}).Info("creating db connection")
...
func NewFromConnection(conn *sqlx.DB, logger log.Logger) *Connection {
	logger.WithFields(log.Fields{"driver": conn.DriverName()}).Info("reusing db connection")
...
func (c *Connection) Close() error {
	c.log.Info("closing db connection")
	return c.DB.Close()
}
...
func logQuery(logger log.Logger, start time.Time, query string) {
	...
	if duration > 100*time.Millisecond || query == "ROLLBACK" {
		logger.WithFields(log.Fields{"time": duration.String()}).Warn(query)
	} else {
		logger.WithFields(log.Fields{"time": duration.String()}).Info(query)
	}
}
